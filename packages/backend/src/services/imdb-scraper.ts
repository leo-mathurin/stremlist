...
import type {SortOptions, StremioMeta, WatchlistData} from "@stremlist/shared";
...
async function getImdbWatchlist(userId: string): Promise<ImdbEdge[] | null> {
  ...
  const titleListItemSearch = pageProps.mainColumnData.predefinedList.titleListItemSearch
...
function convertToStremioFormat(items: ProcessedItem[], sortOptions: SortOptions): StremioMeta[] {
  ...
  for (const item of items) {
    ...
    const isSeries = item.type === "TV Series" || item.type === "TV Mini Series"
...
export async function fetchWatchlist(
  ...
  console.log(`Raw watchlist data received from IMDb for user ${imdbUserId} (${edges.length} items)`)
  ...
  console.log(`Converted ${metas.length} items to Stremio format (sorted by ${sortOptions.by}, ${sortOptions.order})`)
